				...
				<div className='container-fluid'>
					...
					<div className='col-md-6 ml-auto mr-auto' id='search-form'>
						<div className='row'>
							...
							<div className='col-6 d-flex justify-content-end'>
								<h5 className='d-inline booking-text'>
									Refundable - {booking.refundable && <span> "Yes" </span>}
						...
						<table className='table table-bg table-bordered table-sm'>
							<thead>
								<tr>
									...
									<th> Ticket No </th> <th> Cancel ticket </th>
								</tr>
							</thead>
							<tbody>
									...
									booking.passengers.map((passenger) => {
										return (
											<tr>
												...
												<td> {passenger.nationality} </td> <td> {passenger.passenger_type} </td>
												<td> {passenger.ticket_no} </td>
												{passenger.passenger_status === 'verified' && (
													<td>
														<span
															className='btn btn-danger d-flex align-items-end'
															onClick={() => this.cancelUserRequest([passenger])}
														>
															Cancel tickets
														</span>
													</td>
					...
					<div className='col-md-6 ml-auto mr-auto' id='search-form1'>
						...
						<table className='table table-bg table-bordered table-sm'>
							<thead>
								<tr>
									<th> Sector </th> <th> Flight No </th> <th> Flight Time </th> <th> Class </th>
									<th> Flight Charge </th> <th> Cancel all tickets </th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<td> {booking.sector} </td> <td> {booking.flight_no} </td>
									<td> {booking.flight_date} </td> <td> {booking.class_code} </td>
									<td> {booking.total_fare} </td>
									{booking.status === 'verified' && (
										<td>
											<span
												className='btn btn-danger d-flex align-items-end'
												onClick={() => this.cancelUserRequest(booking.passengers)}
											>
												Cancel All tickets
											</span>
										</td>
